#!/usr/bin/env python3
"""Live FoundationPose 6DoF pose estimation with RealSense camera + YOLO detection."""
...
def get_yolo_mask(model, frame, conf=0.5):
    """Run YOLO detection and return binary mask."""
    h, w = frame.shape[:2]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--yolo_model', type=str,
                        default='/home/vbhavanantha/Desktop/PoseEstimation/FoundationPose/gripper_dataset/runs/gripper_detect11/weights/best.pt')
    parser.add_argument('--est_refine_iter', type=int, default=5)
    parser.add_argument('--track_refine_iter', type=int, default=1)
    parser.add_argument('--debug', type=int, default=1)
    parser.add_argument('--debug_dir', type=str, default=f'{code_dir}/debug_realsense')
    ...
    logging.info(f"YOLO loaded from {args.yolo_model}")
    ...
    try:
        while True:
            ...
            if pose is None:
                # Initial detection: use YOLO mask + FoundationPose register
                mask, conf = get_yolo_mask(yolo, color[..., ::-1])  # YOLO expects BGR
                if conf >= 0.5:
