...
split into train/val, create dataset.yaml, and train YOLOv8.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Convert labelme annotations to YOLO format and train YOLOv8."
    )
    parser.add_argument(
        "--images_dir",
        type=str,
        default="gripper_images",
        help="Directory containing labelme .json and .png files.",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default="gripper_dataset",
    ...
    parser.add_argument(
        ...
        type=str,
        default="yolov8n.pt",
        help="Pretrained YOLO model to fine-tune.",
    )
    return parser.parse_args()


def labelme_to_yolo(json_path):
    """Read a labelme JSON file and return a list of YOLO-format label strings.

    Each string is: ``class_id cx cy w h`` with values normalized to [0, 1].
    """
...
def convert_annotations(images_dir, output_dir):
    """Convert all labelme JSONs to YOLO .txt labels and split into train/val.

    Returns the absolute path to the created dataset.yaml.
    """
    # Gather all json files that have a matching image
    ...
    for split_name, split_samples in [("train", train_samples), ("val", val_samples)]:
        for img_path, json_path in split_samples:
            ...
            yolo_lines = labelme_to_yolo(json_path)
...
def train(yaml_path, model_name, epochs, output_dir):
    """Load a pretrained YOLO model and train on the prepared dataset."""
    abs_output = os.path.abspath(output_dir)
    ...
    best_weights = os.path.join(
        project_dir, "gripper_detect", "weights", "best.pt"
    )
    if os.path.isfile(best_weights):
        print(f"\nTraining complete. Best weights saved to:\n  {best_weights}")
    else:
        # Fallback: search for the weights in case ultralytics appended a suffix
        found = glob.glob(os.path.join(project_dir, "gripper_detect*", "weights", "best.pt"))
        if found:
...
def main():
    ...
    # Step 1-3: Convert annotations and build dataset
